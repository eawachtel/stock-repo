...
#Create new dataframe with only the 'Open' column
data = df.filter(['Open'])
...
for i in range(0, days_predicted):
    ...
    last_60_days = df.Open.tail(60 - i).values
...
fig = go.Figure(data=[go.Scatter(x=df.index, y=df['Open'], name='Current Price',
