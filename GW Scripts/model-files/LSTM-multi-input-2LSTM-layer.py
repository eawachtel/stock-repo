...
if trainModel:
    ...
    for i in range(60, len(closeScaledDataTrain)):
        x_close = closeScaledDataTrain[i-60:i, 0]
        x_close_train.append(x_close)
        y_train.append(closeScaledDataTrain[i, 0])

    x_rsi_train = []
    for i in range(60, len(rsiScaledDataTrain)):
        x_rsi = rsiScaledData[i-60:i, 0]
        x_rsi_train.append(x_rsi)

    """Reshape Input Data"""
    x_close_train_array = array(x_close_train)
    x_close_train = x_close_train_array.reshape(x_close_train_array.shape[0], 60, 1)

    x_rsi_train_array = array(x_rsi_train)
    x_rsi_train = x_rsi_train_array.reshape(x_rsi_train_array.shape[0], 60, 1)
    ...
    close = keras.Input(shape=(60, 1), name='close')

    rsi = keras.Input(shape=(60, 1), name='rsi')

    closeLayers = LSTM(10, return_sequences=False)(close)
    rsiLayers = LSTM(10, return_sequences=False)(rsi)

    output = keras.layers.concatenate(
        [closeLayers, rsiLayers, ]
    )
...
if trainModel:
    ...
    while rmse > .5:
        ...
        model.fit([x_close_train, x_rsi_train], [y_train], batch_size=1, epochs=5)
        ...
        for i in range(60, len(test_data)):
            xclose = closeScaledData[i - 60:i, 0]
            xrsi = rsiScaledData[i - 60:i, 0]

            x_test.append([xclose, xrsi])
